...
//Display the rocket.
         ...
         if (gameOver)
        {
            ...
            g.drawString("GAME OVER", getWidth2()/5, getHeight2()/2);

        }
         ///bomb
         if(bomb)
         {
            drawCircle(getX(bombXPos),getYNormal(bombYPos),
                0.0,bombXScale,bombYScale ); 
         }
         
         ///////score
         g.setColor(Color.black);
        g.setFont(new Font("Impact",Font.BOLD,15));
        g.drawString("Score: " + score, 5, getYNormal(getHeight2()));
        
        ///highScore
        g.setColor(Color.black);
        g.setFont(new Font("Impact",Font.BOLD,15));
        g.drawString("High Score: " + highscore, getWidth2()/2-50, getYNormal(getHeight2()));
        
        ///lives
         g.setColor(Color.black);
        g.setFont(new Font("Impact",Font.BOLD,15));
        g.drawString("Lives: " + lives, getWidth2()/2 +150, getYNormal(getHeight2()));
...
//Code for missiles hitting stars.
        ...
        for (int j=0;j<numStars;j++)
            {
                if (bomb && bombXPos+10 > starXPos[j] &&
                bombXPos-50 < starXPos[j] &&
                bombYPos+50 > starYPos[j] &&
                bombYPos-50 < starYPos[j])
                {
                    score +=5;
        ...
        try {
            ...
            while (line != null) {
                ...
                if (newLine.startsWith("nummissiles"))
                {
                    ...
                    Missile.numMissiles = Integer.parseInt(numStarsString.trim());
                }
                line = in.readLine();
            }
            in.close();
...
//Class variables.
    ...
    public static int numMissiles = 15;     
//Instance variables.
    public int xpos;
    public int ypos;
